...
# ----------------------------------------
def save_sample_png(sample_folder, sample_name, img_list, name_list, imgrange, imglow, pixel_max_cnt = 255, height = -1, width = -1):
    ...
    for i in range(len(img_list)):
        ...
        #逆归一化
        img_copy = img_copy[0,0,:,:] * im_range + im_low

        # Save to certain path
        save_img_name = sample_name + '_' + name_list[i]
        save_img_path = os.path.join(sample_folder, save_img_name)

        aa = img_copy[img_copy > 255]
        b = img_copy[img_copy < 0]
        np.save(save_img_path, img_copy.detach().cpu().numpy())
        # cv2.imwrite(save_img_path, img_copy)

    return img_copy

def save_sample_png_train(sample_folder, sample_name, img_list, name_list, imgrange, imglow, pixel_max_cnt = 255, height = -1, width = -1):
    if not os.path.exists(sample_folder):
        os.mkdir(sample_folder)
    ...
    for i in range(len(img_list)):
        ...
        b = img_copy[img_copy < 0]
        np.save(save_img_path, img_copy.detach().cpu().numpy())
        # np.save(save_img_path, img_copy.detach().numpy())
        # cv2.imwrite(save_img_path, img_copy)

    return img_copy

def save_sample_png_test(sample_folder, sample_name, img_list, name_list, imgrange, imglow, pixel_max_cnt = 255):
    # Save image one-by-one
    for i in range(len(img_list)):
        img = img_list[i]
        # Recover normalization
        ...
        #逆归一化
        img_copy = img_copy * imgrange + imglow
        # Save to certain path
        save_img_name = sample_name + '_' + name_list[i] + '.png'
        save_img_path = os.path.join(sample_folder, save_img_name)
        # cv2.imwrite(save_img_path, img_copy)
        np.save(save_img_path, img_copy.detach().numpy())
def recover_process(img, imgrange, imglow, height = -1, width = -1):
    img = img * 255.0
    img_copy = img
    img_copy = img_copy * imgrange + imglow
