...
class MISF():
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # imrange = []
                # imlow = []
                images, masks, imrange, imlow = self.cuda(*items)
                ...
                if iteration % self.config.TRAIN_SAMPLE_INTERVAL == 0:
                    ...
                    kpn_utils.save_sample_png_train(sample_folder=self.config.TRAIN_SAMPLE_SAVE,
                                              sample_name='ite_{}_{}'.format(self.inpaint_model.iteration,
                                                                             0), img_list=img_list2,
                                              name_list=name_list2, imgrange=imrange, imglow=imlow, pixel_max_cnt=255, height=-1,
                                              width=-1)
                ...
                if iteration % self.config.EVAL_INTERVAL == 0:
                    print('\nstart eval...\n')
                    cur_psnr, avssim, avloss = self.eval()
                    self.inpaint_model.iteration = iteration

                    logg = str(cur_psnr)+' '+str(avssim)+' '+str(avloss)

                    Note = open('note.txt', mode= 'a')
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            return np.average(psnr_all), np.average(ssim_all), np.average(l1_list)
            ...
            for items in test_loader:
                ...
                if len(ssim_list) % 1 == 0:
                    ...
                    kpn_utils.save_sample_png(sample_folder=self.config.TEST_SAMPLE_SAVE, sample_name='{}_'.format(len(ssim_list)),
                                              img_list=img_list,
                                              name_list=name_list, imgrange=imrange, imglow=imlow,  pixel_max_cnt=255, height=-1, width=-1)
